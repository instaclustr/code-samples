...
public class DroneDeliveryApp2 {
...
public static class DroneWorkflowImpl implements DroneWorkflow {
        ...
        public String startWorkflow(String name) {
            ...
            // blocking call waits for order from kafka - currently fake, order is built here!
            // assumes there is an order name as value however
            // TODO Check timeout and exceptions for this call...
            ...
            */
            ...
            OrderWorkflow exampleWorkflow = workflowClient.newWorkflowStub(OrderWorkflow.class);
            // This works, but shouldn't need to start the wf as it's already running!
        	WorkflowExecution workflowExecution = WorkflowClient.start(exampleWorkflow::startWorkflow, orderID);
        	orders.add(exampleWorkflow);
        	orderIDs.add(workflowExecution.getWorkflowId());
        	
        	exampleWorkflow.signalOrder("droneHasOrder");
            
            System.out.println("Drone " + name + " has got order " + orderIDs.get(0) + " gogogo!");
            
            // TODO Really the order should be created, or the order should create, the order and delivery location
            // And the Drone WF should get them and construct flight plan 
            
            LatLon orderLoc = exampleWorkflow.getOrderLocation();
            LatLon deliveryLoc = exampleWorkflow.getDeliveryLocation();
            ...
            // TODO Add something "interesting" here? if drone failed to reach order location for some reason, give up, put order onto priority job topic, and return to base.
            // could also do same for pickup - i.e. something could go wrong even once drone has arrived at location.
    ...
    public static class DroneActivitiesImpl implements DroneActivities
    {
    	...
    	private int count = 0;
         ...
         public void nextLeg(LatLon start, LatLon end, boolean updateOrderLocation, String orderID)
         {
			 ...
			 DroneDeliveryApp2.DroneWorkflow droneWF = workflowClient.newWorkflowStub(DroneWorkflow.class, id);
			 
			 DroneDeliveryApp2.OrderWorkflow orderWF = null;
       ...
       //    private void locationLoopAsync(WorkflowExecution execution, ArrayList<OrderWorkflow> orders)
         private void locationLoopAsync(WorkflowExecution execution, ArrayList<String> ids)
         {
        	 ...
        	 DroneDeliveryApp2.OrderWorkflow stub;
        	...
        	// DroneDeliveryApp1.DroneWorkflow stub = workflowClient.newWorkflowStub(DroneWorkflow.class, id);
        	 ...
        	 while (true)
        	 {
            	 ...
            	 */
        		 ...
        		 try {
        			 String id = execution.getWorkflowId();   	
        			 DroneDeliveryApp2.DroneWorkflow stub2 = workflowClient.newWorkflowStub(DroneWorkflow.class, id);
